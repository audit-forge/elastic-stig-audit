"""Terminal report renderer for elastic-stig-audit."""
...
def render(results, target_info, summary=None):
    summary = summary or {}
    print("elastic-stig-audit — CIS Elasticsearch Container Security Assessment")
    ...
    for r in sorted(
        ...
        if r.remediation and r.status.value not in ("PASS", "SKIP"):
            # Print first line of remediation only
            first_line = r.remediation.split("\n")[0]
            print(f"  Fix:      {first_line}")
        if r.fedramp_control or r.nist_800_53_controls:
            print(
                f"  Mapping:  fedramp={r.fedramp_control or '-'} | "
                f"nist={', '.join(r.nist_800_53_controls) if r.nist_800_53_controls else '-'}"
            )
        if r.cve_ids:
            print(f"  CVEs:     {len(r.cve_ids)} found | KEV: {r.kev_score or 'none'}")
        if r.evidence:
            print(f"  Evidence: {len(r.evidence)} item(s) captured")
        print()
